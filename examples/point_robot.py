...
from obstacle_constraints import get_linearized_obstacle_constraints
...
def run_point_robot(n_steps=1000, render=False, goal=True, obstacles=True):
    ...
    for _ in range(n_steps):
        ...
        robot_pos = ob['robot_0']['joint_state']['position'][0:2]
        print(get_linearized_obstacle_constraints(env, robot_pos))
        if terminated:
            print(info)
            break
        history.append(ob)
...
if __name__ == "__main__":
    run_point_robot(render=True, goal=False)
