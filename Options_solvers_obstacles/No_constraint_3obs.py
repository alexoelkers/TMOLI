#For this solver we don't add a constraint for the obstacle, we add the obstacles directly to the cost function. IT WORKS (3 Static obstacles)
...
def main():
    ...
    for i in range(0, N):
        ...
        dist_to_obstacle3 = (state[0] - x_obs3)**2 + (state[1] - y_obs3)**2  # squared distance to third obstacle
        penalty3 = cs.fmax(0, r_obs3**2 - dist_to_obstacle3)  # Penalize if inside the third obstacle

        # Combine the three penalties
        obstacle_penalty.append(penalty1 + penalty2 + penalty3)
